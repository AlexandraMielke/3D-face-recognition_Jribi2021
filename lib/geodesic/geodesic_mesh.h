/*
 * SPDX-FileCopyrightText: 2008 Danil Kirsanov
 *
 * SPDX-License-Identifier: MIT
 */
/*
 * SPDX-FileCopyrightText: 2024 Alexandra Mielke
 *
 * SPDX-License-Identifier: MIT
 */
//Mielke 2024: Added average_edge to class mesh
...
{
	if(p->type() == UNDEFINED_POINT) throw std::runtime_error("Error: Point type not defined.");
	if(p->type() == VERTEX)
	{
		if(storage)
		{
								...
								Faces& tri)
{
	...
	for(unsigned i=0; i<num_faces; ++i)		//copy adjacent vertices to polygons/faces
	{
		...
		{
			unsigned vertex_index = tri[i][j]; //shift + j]; //access index of the triangles 3 vertices
			if(vertex_index >= num_vertices)
			{
				throw std::runtime_error("Error: vertex index exceeds the number of vertices");
			}
...
/**
 * Generats vectors with adjacent faces and edges.
 * 
 * @param None
 * @return bool `True` if adjacencies could be built, `False` if it fails.
 */
inline void Mesh::build_adjacencies()
{
	//Vertex->adjacent Faces
	std::vector<unsigned> count(m_vertices.size());	//emtpy container to count adjacent vertices
	for(unsigned i=0; i<m_faces.size(); ++i) //count adjacent faces per vertice
	{
		Face& f = m_faces[i]; 
		for(unsigned j=0; j<3; ++j)
		{
	...
	//std::cout << "Max adjacent faces: " << *std::max_element(count.begin(), count.end()) << std::endl;
	//std::cout << "Min adjacent faces: " << *std::min_element(count.begin(), count.end()) << std::endl;
	...
	std::fill(count.begin(), count.end(), 0);  //reset count to all zeros
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)
		{
			...
			v->adjacent_faces()[count[v->id()]++] = &f; //count adjacent faces per vertice
		}
	}

	//find all edges
	//i.e. find all half-edges, sort and combine them into edges
	std::vector<HalfEdge> half_edges(m_faces.size()*3); //always 3 half edges per face
	unsigned nr_halfedges = 0;
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		Face& f = m_faces[i];
		for(unsigned j=0; j<3; ++j)
		{
			half_edges[nr_halfedges].face_id = i;
			unsigned vertex_id_1 = f.adjacent_vertices()[j]->id();
			unsigned vertex_id_2 = f.adjacent_vertices()[(j+1) % 3]->id();
			half_edges[nr_halfedges].vertex_0 = std::min(vertex_id_1, vertex_id_2);
			half_edges[nr_halfedges].vertex_1 = std::max(vertex_id_1, vertex_id_2);
	...
	unsigned faulty_edges = 0;
	for(unsigned i=1; i<half_edges.size(); ++i) //iterate through half edges and count unique edges
	{
		if((half_edges[i].vertex_0 != half_edges[i-1].vertex_0) 
		|| (half_edges[i].vertex_1 != half_edges[i-1].vertex_1))
		{ //this and the previous half edge are not identical
				++number_of_edges;
		}
		else
		{
			...
			{								//if it fails, most likely the input data are messed up
				if(!((half_edges[i].vertex_0 != half_edges[i+1].vertex_0) || (half_edges[i].vertex_1 != half_edges[i+1].vertex_1)))
				{
					throw std::runtime_error("Error: More than two equal half-edges detected.");
				}
				//assert((half_edges[i].vertex_0 != half_edges[i+1].vertex_0) || (half_edges[i].vertex_1 != half_edges[i+1].vertex_1));
			}
		}
	}
	...
	for(unsigned i=0; i<half_edges.size();)
	{
		...
		if(e.length() < 1e-100)	throw std::runtime_error("Degenerate edge detected.");
		...
		{
			e.adjacent_faces().set_allocation(allocate_pointers(1),1);		//one adjacent faces
			e.adjacent_faces()[0] = &m_faces[half_edges[i].face_id];
			i += 1;
		}
	}
	...
	std::fill(count.begin(), count.end(), 0); //reset count again
	for(unsigned i=0; i<m_edges.size(); ++i)
	{
		Edge& e = m_edges[i];
		if(e.adjacent_vertices().size() != 2)
		{
			throw std::runtime_error("Error: Expected 2 adjacent vertices in edge, but found " + std::to_string(e.adjacent_edges().size()));
		}
	...
	std::fill(count.begin(), count.end(), 0);
	for(unsigned i=0; i<m_edges.size(); ++i) //iterate through all unique edges and assign them to face
	{
		Edge& e = m_edges[i];
		for(unsigned j=0; j<e.adjacent_faces().size(); ++j)
		{
			face_pointer f = e.adjacent_faces()[j];
			if(count[f->id()] >= 3) throw std::runtime_error("Error: Expected 3 adjacent edges in face, but found more.");
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)		//compute angle adjacent to the vertex j
		{
			...
			if(angle <= 1e-5)	throw std::runtime_error("Degenerate face detected.");  
			f.corner_angles()[j] = angle;
			sum += angle;
		}
		//algorithm works well with non-degenerate meshes only 
		if(std::abs(sum - M_PI) > 1e-5)	throw std::runtime_error("Degenerate face detected.");
	}
	...
	for(unsigned i=0; i<m_edges.size(); ++i)
	{
		...
		if(e.is_boundary())
		{
			...
			e.adjacent_vertices()[0]->is_boundary() = true;
			e.adjacent_vertices()[1]->saddle_or_boundary() = true;
			e.adjacent_vertices()[1]->is_boundary() = true;
		}
	}
	if(!verify()) throw std::runtime_error("Mesh is not connected correctly.");
}

/**
 * Verify connectivity of the mesh and print debug info.
 */
inline bool Mesh::verify()
{
	...
	{
		throw std::runtime_error("There are unconnected vertices in the mesh.");
	}
	...
	std::vector<face_pointer> stack(1,&m_faces[0]); //stack with size 1 as element adress to first face
	...
	while(!stack.empty())
	{
		face_pointer f = stack.back(); //read last element
		stack.pop_back(); //delete last element

		for(unsigned i=0; i<3; ++i) //iterate through all edges of face (max 3)
		{
			edge_pointer e = f->adjacent_edges()[i]; 
			face_pointer f_adjacent = e->opposite_face(f); //pointer to other face of this edge
			if(f_adjacent && !map[f_adjacent->id()]) //only get unique connected faces
			{
				map[f_adjacent->id()] = true;
				stack.push_back(f_adjacent); //stack all unique connected faces
			}
		}
	}
	auto iter = map.begin();
	while ((iter = std::find(iter, map.end(), false)) != map.end())
	{
	...
	if(std::find(map.begin(), map.end(), false) != map.end())
	{
		throw std::runtime_error("Mesh is not connected entirely. Please delete lonely faces or add connecting edges.");
	}
	...
	for(unsigned i=0; i<m_edges.size(); ++i)
	{
		...
		this->average_edge += e.length();
	}
	this->average_edge /= m_edges.size();
