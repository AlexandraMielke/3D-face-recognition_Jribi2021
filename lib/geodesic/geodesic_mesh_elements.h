...
public:
	...
	bool& is_boundary(){return m_is_boundary;};
private:					
	...
	bool m_is_boundary;				//true if vertex belongs to the mesh boundary
};
...
public:
	...
	face_pointer opposite_face(face_pointer f) //returns ID of other face that shares this edge
	{
		if(adjacent_faces().size() == 1)
		{
			//if edge only belongs to a single face
			assert(adjacent_faces()[0]->id() == f->id());
			return NULL;
		}

		//Either of the adjacent faces must be current face
		assert(adjacent_faces()[0]->id() == f->id() || 
